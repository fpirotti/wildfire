...
for(area in names(todos) ){
  ...
  cm_ext <- cbind(cm_ext, "Prod.Acc" = sprintf("%.1f%%", prod_acc*100)) 
  # Add User’s Accuracy as extra row
  cm_ext <- rbind(cm_ext, "User.Acc" = c(sprintf("%.1f%%", user_acc*100), 
                                         sprintf("%.1f%%", overall_acc*100)))
  
  
  k <- kable(cm_ext, caption = paste0("--", area, " - Confusion Matrix ALL rows=CzechGlobe (reference)   colums=Wildfire-V2 (Classified)   kappa=", round(kappa,3)) ) %>%
  kable_styling(latex_options = c("scale_down", "hold_position"),full_width = FALSE, font_size =9) %>%
...
k <- kable(cm_ext, align = "r", caption = paste0("--", area, " - Confusion Matrix rows=CzechGlobe (reference)   colums=Wildfire-V2 (Classified)   kappa=", round(kappa2,3)) ) %>%
